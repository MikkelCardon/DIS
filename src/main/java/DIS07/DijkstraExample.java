...
class DijkstraExample {
    ...
    public static void main(String[] args) {
        ...
        graph.put("B", Arrays.asList(new Edge("A", 6), new Edge("D", 2), new Edge("C", 5), new Edge("E", 2)));
        graph.put("C", Arrays.asList(new Edge("B", 5), new Edge("E", 5)));
        graph.put("D", Arrays.asList(new Edge("A", 1), new Edge("E", 1), new Edge("B", 2)));
        graph.put("E", Arrays.asList(new Edge("D", 1), new Edge("C", 5), new Edge("B", 2)));
        ...
        for (Map.Entry<String, Integer> entry : result.entrySet()) {
            System.out.println("Korteste afstand fra A til " + entry.getKey() + " = " + entry.getValue());
        }
    }
}